...
3. 进入测试用例页面，说明用例按接口、UI、功能、异常、安全等维度组织。
4. 进入自动化执行页面，选择场景触发后端 pytest 执行。
5. 进入测试报告页面，展示 JUnit / Allure 产物和执行摘要。
6. 进入知识库管理页面，说明需求、接口文档、历史 Bug、日志和测试报告可以作为 RAG 来源。
7. 进入 AI 测试助手，选择“智能用例生成”或“失败原因分析”模板，展示结构化输出。
8. 最后说明 MCP tools 把这些能力封装成可编排工具，可以被 Agent 调用。
...
> 这里参考了 Postman / Apifox / 测试平台常见结构，但没有照搬。左侧是接口目录，中间是接口列表，右侧是接口详情，包括 method、path、headers、request body、assertions、关联测试用例、pytest 目标和最近执行状态。
...
- 构建 API 测试中心，维护接口 method、path、headers、request body、assertions、关联测试用例和 pytest 执行目标。
...
3. 增加接口调试能力，支持用户输入 URL、headers、body 和断言。
4. 增加测试计划，把多个场景组合成一次执行。
5. 增加失败趋势图、模块质量分布、历史执行对比。
6. 增加用例编辑和导出能力。
