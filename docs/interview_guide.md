...
4. 进入 `追踪矩阵`，说明平台会把需求项拆出来，并匹配生成的测试点、自动化场景和最近执行状态。这里可以回答“每条需求有没有测试覆盖、有没有自动化、最近跑没跑过、缺口在哪里”。
...
面试官经常会问“你的测试用例覆盖了哪些需求，哪些已经自动化，执行结果是什么”。只展示测试点或测试报告是不够的，因为两者之间缺少关系。追踪矩阵把需求项、测试点、自动化场景和最近执行记录放到同一行里，能直接看出覆盖率、自动化率、最近通过率和缺口，比如缺少测试设计、没有自动化、只做了 dry-run 或最近执行失败。
...
下一步我会优先给追踪矩阵增加导出能力和人工确认状态，让它可以变成评审材料。之后可以补轻量性能基线，比如 RAG 检索、报告解析、执行历史趋势聚合的耗时统计，再接 MCP browser 工具或真实业务测试站点。
...
- 构建需求-测试点-执行结果追踪矩阵，将需求项关联到测试设计、自动化场景和最近执行历史，展示覆盖率、自动化率、通过率和缺口。
