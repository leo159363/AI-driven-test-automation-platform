...
9. 进入 `执行历史`，查看保存过的执行计划记录、状态、失败原因、报告路径和截图 artifact。
...
自动化测试平台不能只展示“刚刚跑了一次”的结果，还要能回答什么时候跑过、跑的哪个场景、失败原因是什么、报告和截图在哪里。当前版本用 JSONL 存储轻量执行记录，后续可以自然升级成数据库任务表、趋势分析和定时任务。
...
- 增加执行历史记录，保存场景、状态、失败原因、报告路径和截图 artifact，让平台具备测试任务追踪能力。
```
