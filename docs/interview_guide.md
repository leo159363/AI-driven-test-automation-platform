...
测试开发不只是写脚本，还要看执行质量。项目的报告中心能解析 JUnit XML，并发现 Allure 目录。自动化场景和执行计划 adapter 都可以产生 JUnit XML，这样可以把自动化执行、自然语言执行计划和测试报告展示串起来，面试时能讲完整链路。
...
- 将执行适配器结果转换为 JUnit XML，使自然语言执行计划结果能进入统一测试报告中心展示。
```
