...
3. `自动化场景`：查看内置 API / UI 自动化示例。
4. `执行计划`：选择 API 登录或 UI 登录场景，生成自然语言步骤对应的执行计划，可开启 dry-run。
5. `执行历史`：查看保存过的执行计划记录、通过率趋势、状态分布、失败原因和报告路径。
6. `测试报告`：查看 JUnit XML 和 Allure 目录状态。
7. `测试设计评估`：运行 Golden Test Set，展示测试点生成质量指标。
8. `评估中心`：说明已有 RAG 评估入口。
...
> 我这个项目是一个面向测试开发场景的 AI 自动化测试平台。它不是只做一个页面 demo，而是把需求输入、RAG 知识检索、测试点生成、测试设计评审、质量评估、自动化场景执行、执行历史、测试报告展示和执行计划适配串成一个闭环。核心思路是：测试人员输入需求或接口说明后，系统结合需求文档、API 文档、缺陷记录、测试规范和执行日志生成结构化测试点；再用规则评审检查缺失维度、空泛描述和不可执行断言；再用 Golden Test Set 评估生成质量；自动化侧内置 API 和 UI 场景，能输出 JUnit / Allure 兼容报告；最后通过自然语言执行计划接入 API HTTP adapter 和 Browser UI adapter，展示从“生成计划”到“执行记录和报告中心”的路径。
...
- 增加需求、测试点、执行结果之间的 traceability matrix。
